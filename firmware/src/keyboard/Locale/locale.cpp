...
// Define a type for the function pointers. int (not uint8_t) so non-Latin
// locales can return Unicode code points above 255.
...
int keyboard_keycode_ascii(String locale, uint8_t keycode, bool shift, bool alt, bool pressed)
{
  ...
  static const std::unordered_map<String, KeycodeFunction, StringHash> locale_map = {
      ...
      {"AR", keyboard_keycode_ascii_ar},
      {"ARW", keyboard_keycode_ascii_ar_win},
  };
...
{
  if (locale == "ARW")
    return keyboard_ar_win_ligature(keycode, shift, c1, c2);
  return false;
}
