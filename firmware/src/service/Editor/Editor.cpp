...
bool Editor::loadWindow(size_t offset, size_t length)
{
    ...
    // block read - a per-byte read() loop through the VFS layer takes hundreds
    // of ms for a half-buffer window, long enough for the auto-repeat engine to
    // re-trigger paging mid-load
    if (length > BUFFER_SIZE)
        length = BUFFER_SIZE;
    int r = file.read((uint8_t *)buffer, length);
    size_t bytesRead = (r < 0) ? 0 : (size_t)r;
    buffer[bytesRead] = '\0';
...
void Editor::pageBackward()
{
    ...
    // stop auto-repeat so the display core can't re-enter paging while this
    // (slow, file I/O) load is still running
    lastKey = 0;
    lastPressTime = 0;
...
void Editor::advanceWindow()
{
    ...
    // stop auto-repeat so the display core can't re-enter paging while this
    // (slow, file I/O) window swap is still running
    lastKey = 0;
    lastPressTime = 0;
