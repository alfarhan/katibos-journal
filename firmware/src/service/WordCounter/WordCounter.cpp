...
// A byte counts as part of a word if it is alphanumeric ASCII or any non-ASCII
// (>= 0x80) byte - the latter covers UTF-8 multibyte scripts like Arabic, whose
// letters would otherwise be treated as separators and never counted. Spaces,
// newlines and ASCII punctuation still break words. Cast to unsigned to avoid
// passing a negative char to isalnum (undefined behaviour).
static inline bool is_word_byte(char c)
{
    unsigned char b = (unsigned char)c;
    return std::isalnum(b) || b >= 0x80;
}
